...
if len(trim(request.querystring("tda"))) > 0 then tda = request.QueryString("tda")
marka = 0
    CAD =   " SELECT * FROM  VIEW_ARTICULOS_TIENDA   WHERE   " & _
            " tienda = '"&TDA&"' AND codigo like '"&POS&"%' and estado = 'A'  "  
            'and stock>0  " & _
	cad = cad + 		" order by codigo; "  ' Fac 20121229
           ' response.write(cad)
           ' response.write("<br>")
    RS.OPEN CAD,CNN
    if rs.recordcount <= 0 then 
        RESPONSE.WRITE("NO HAY STOCK CARGADO PARA ESTA TIENDA")
        RESPONSE.END
    END IF    %>
    <table width="100%">
        ...
        <%DO WHILE NOT RS.EOF%>
            <tr  bgcolor="<% if CONT mod 2  = 0 THEN 
	                ...
	                response.write(Application("color1"))
	                 ...
	                 id="fila<%=Trim(Cstr(cont))%>" onDblClick="selecta('<%=rs("codigo")%>')">
