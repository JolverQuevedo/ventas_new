...
var largo3 = 10 // es el largo de la clave
var oldrow =1
var olddata =''
var chk = ''
</script>
...
// **************************************************************
var funcionalidad = 'comun/INSERUSR.asp?'
...
'****************************************************
CAD =	" SELECT top  "&pagesize&" USUARIO,     " & _ 
        " NOMBRES, CLAVE, PERFIL                " & _
		" from usuarios                         " & _
        " WHERE USUARIO >= '"&POS&"'            " & _
        " ORDER BY "& indice &"                 " 
        ' response.Write(cad)
...
<table border="0" align="center"  cellspacing="5">
	<tr valign="top">
		<td > 
			<img src="images/primera.gif" style="cursor:pointer;"
			onclick="primera('<%=urlBase%>')" 
			alt="PRIMERA PAGINA" />
		</td>
		<td >
			<img src="IMAGES/PREV.GIF" alt="PAGINA ANTERIOR"
				onclick="atras(alias, '<%=indice%>')" 
				style="CURSOR:pointer" /> 
		</td>
		<td >
			<img src= "images/arriba.gif" alt="REGISTRO ANTERIOR"
				onclick="retrocede()" style="CURSOR:pointer" /> 
		</td>
		<td >
			<img src="images/abajo.gif" alt="REGISTRO SIGUIENTE"
				onclick="avanza()"  style="CURSOR:pointer" /> 
			
		</td>
		<td>
			<img src="images/next.gif" alt="PAGINA SIGUIENTE"
		...
		<td>
			<img src= "images/ultima.gif" alt="ULTIMA PAGINA"
			 onclick="ultima()" 
			style="cursor:pointer;" /> 
		</td>
	<td>		
		<img src="images/PRINT.gif" alt="IMPRESION"
			onclick="imprime()"
			style="cursor:pointer;" />	
	</td>
...
<table	border="0" id="DATAENTRY" align="center" >
   <tr>
   		<td>
            <table	align="center" width="100%" cellpadding="1" cellspacing="0"  bgcolor="WHITE" border="1" >
                  <tr valign="middle"> 
                    <td bgcolor="<%=(Application("barra"))%>" align= "right" class="Estilo8">
                      USER :&nbsp;</td>
                    ...
                    <td bgcolor="<%=(Application("barra"))%>" align= "right" class="Estilo8">
                      PWD :&nbsp;</td>
                    <td bgcolor="WHITE"  valign="middle"> 
                      <input type="text" id="PWD" name="PWD" class="DATOSGRANDE"  maxlength="10"/>
                    </td>
                 </tr>
                 <tr>
                    <td bgcolor="<%=(Application("borde"))%>" align="right" class="Estilo8">DESCRIPCION :&nbsp;</td>
                    <td bgcolor="#FFFFFF" > 
                      <input type="text" id="DES" name="DES"  size="50" />
                    </td>
                    <td  bgcolor="<%=(Application("borde"))%>" align="right" class="Estilo8">Perfil :&nbsp;</td>
                    <td bgcolor="#FFFFFF" > 
                        <select id="DCT" name="DCT">
...
function llena(pos) {
    var t = document.all.TABLA;
    thisForm.PWD.value = ltrim(t.rows(pos).cells(2).innerText);
    var subcadena = trim(t.rows(pos).cells(3).innerText)
    var elemento = thisForm.DCT;
    thisForm.DCT.selectedIndex = seleindice(subcadena, elemento);
...
function NUEVO_onclick() {
    ...
    thisForm.COD.value = '';
    thisForm.DES.value = '';
    thisForm.DCT.selectedIndex = -1;
    thisForm.PWD.value = '';
    document.all.COD.focus();
}
function completa() {
    CAD = 'per=' + thisForm.DCT.value
    CAD +='&pwd=' + thisForm.PWD.value
     return CAD
}
function VALIDA() {
    ...
    if (trim(window.thisForm.DCT.value) == "") {
        alert('Debe Informar el tipo de usuario');
        window.thisForm.DCT.focus();
        return false;
    }
    if (trim(window.thisForm.PWD.value) == "") {
        alert('No se aceptan claves en blanco');
        window.thisForm.PWD.focus();
        return false;
    }
    return true;
}
