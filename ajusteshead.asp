...
<table id="Table1" align="center"  bordercolor="#FFFFFF"  bgcolor="<%=Application("color2")%>"  cellpadding="1"  cellspacing="2"  border="0" align="center" >
    <tr valign="middle"><td class="Estilo1">Número :</td><td class="Estilo1">Motivo</td><td class="Estilo1">Sustento</td></tr>
    <tr valign="middle" >
    <% cad = "select serie, correl from documento where cia = '"&tienda&"' and codigo = 'AJ'"
        rs.open  cad, cnn
        if rs.recordcount > 0 then 
                rs.movefirst
        else
           response.Write("Favor Verificar que exista el tipo de Documento AJ creado para esta tienda")                 
           response.End
        end if%>
        ...
        RS.OPEN CAD,CNN%>
        <%IF RS.RECORDCOUNT <= 0 THEN%>
            <option value="">No Hay Motivos definidos a partir del 50</option>
        <%END IF%>
        <%rs.movefirst
        do while not rs.eof%>
...
function manda() {
    ...
    parent.window.frames[1].window.location.replace("AJUSTESdeta.asp")
 return true
}
